...
<form id="loginn" action="/login" method="POST">
  ...
  <label for="purp">Admin/Student ID</label>
          ...
          $.ajax({
            ...
            success: function(data) {
              if (data.redirect) {
                ...
                alert(data.error); // Display the error message in a pop-up
              } else {
                // Fallback: Display the response text in the #ajax_success container
                $('#ajax_success').html(data.text);
              }
            },
            error: function(data) {
              // Show a generic error message for failed AJAX requests
